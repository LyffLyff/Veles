...
func load_option(var option_idx : int) -> void:
	...
	for n in option_vbox.get_child_count():
		...
		if option_vbox.get_child(n).get_child(2).connect("mouse_exited",self,"on_option_mouse_exited"):
		...
		if option_vbox.get_child(n).get_child(2) is ColorPickerButton:
			if option_vbox.get_child(n).get_child(2).connect("pressed",self,"on_color_changer_shown",[n]):
				Global.root.message("COULDN'T CONNECT picker_created SIGNAL TO on_color_changer_shown FUNCTION",  SaveData.MESSAGE_ERROR )
			if option_vbox.get_child(n).get_child(2).connect("popup_closed",self,"on_color_changer_freed"):
				Global.root.message("COULDN'T CONNECT popup_closed SIGNAL TO on_color_changer_freed FUNCTION",  SaveData.MESSAGE_ERROR )
			if option_vbox.get_child(n).get_child(2).connect("color_changed",self,"on_color_changed"):
				Global.root.message("COULDN'T CONNECT color_changed SIGNAL TO on_color_changed FUNCTION",  SaveData.MESSAGE_ERROR )
...
func on_option_mouse_exited() -> void:
	settings_animations.play_backwards("InfoInOut")
