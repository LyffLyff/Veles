...
onready var sub_options : VBoxContainer = $MarginContainer/VBoxContainer/Options
onready var user_profile_container : PanelContainer = $MarginContainer/VBoxContainer/UserProfileBox
...
func _ready():
	...
	self.get_stylebox("bg").set(
		"bg_color",
		SettingsData.get_setting(SettingsData.DESIGN_SETTINGS,"SidebarBackground")
	)
	
	# hiding Scrollbar
	self.get_v_scrollbar().modulate = "00000000"
	self.get_v_scrollbar().hide()
