...
static func get_playlist_name(var playlist_idx : int = -1) -> String:
	if playlist_idx == -1:
		...
		var temp_playlist_title = SettingsData.get_setting(SettingsData.GENERAL_SETTINGS, "TemporaryPlaylistTitle")
		if temp_playlist_title != null:
			return temp_playlist_title
		else:
			return ""
	elif playlist_idx <= -3:
		# smart Playlist
...
static func get_playlist_cover(var playlist_idx : int) -> Texture:
	...
	return ImageLoader.get_cover(cover_path, ImageLoader.ImageTypes.PLAYLIST_HEADER)
...
static func copy_playlist_cover(var img_src : String,var playlist_idx : int,var return_texture : bool = false):
	...
	if dir.file_exists(img_src):
		if dir.copy(img_src, cover_path) != OK:
			Global.message("COPYING PLAYLIST COVER TO USERDATA FROM: " + img_src + " TO: " + cover_path,Enumerations.MESSAGE_ERROR, false)
...
static func rename_playlist(var new_title : String, var playlist_idx : int) -> void:
	...
	if playlist_idx >= 0:
		...
		if dir.rename(
			...
			Global.message("RENAMING SmartPlaylistConditions FROM: " + old_title + " TO " + new_title,Enumerations.MESSAGE_ERROR, false)
	...
	if dir.rename(
		...
		Global.message("RENAMING Playlist's Cover FROM: " + old_title + " TO " + new_title,Enumerations.MESSAGE_ERROR, false)
