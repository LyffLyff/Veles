...
var _downloader: Downloader = Downloader.new()
...
func _execute_on_thread(arguments: Array) -> void:
	...
	if playlist:
		if SettingsData.get_setting(SettingsData.GENERAL_SETTINGS, "OverrideTitleOnPlaylistDownload") == false:
			file_path = "{destination}{file_name} - %(title)s.%(ext)s"
		else:
			file_path = "{destination}%(title)s.%(ext)s"
	else:
